...
export default function AIPairingSuggestionsSection({ onNavigateTab }: AIPairingSuggestionsSectionProps) {
  const cards = [
    {
      id: "card-1",
      bgColor: "bg-[#FFF8F0]", // Warm cream/peach pastel
    ...
    {
      id: "card-2",
      bgColor: "bg-[#F2ECFA]", // Soft lavender/purple pastel
    ...
    {
      id: "card-3",
      bgColor: "bg-[#FFF9F2]", // Light warm almond pastel
  ...
  return (
    <div className="w-full bg-[#FAF9FD] rounded-[36px] p-6 sm:p-8 lg:p-10 border border-slate-100 shadow-[0_4px_30px_rgba(0,0,0,0.02)] space-y-9 font-sans text-left relative overflow-hidden">
      
      {/* Soft Purple Radial Glow at Top Right */}
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-gradient-to-bl from-purple-200/40 via-purple-100/20 to-transparent rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="relative flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 z-10 pt-2">
        ...
        <div className="space-y-3.5 max-w-xl">
          ...
          <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.25em] text-[#524375]">
            <Sparkles className="w-3.5 h-3.5 text-[#3B1E78]" />
        ...
        <div className="hidden md:flex items-center gap-6 shrink-0 relative">
          ...
          <div className="text-right font-serif italic text-lg sm:text-xl text-[#6B5A94] font-medium leading-[1.15] relative pr-2">
            <span>Same<br />Ingredients<br />New Possibilities</span>
            <div className="w-12 h-[1.5px] bg-[#6B5A94]/40 rounded-full mt-1.5 ml-auto" />
          </div>

          {/* Top Right Plate Graphic with Lavender Backing Disc */}
          <div className="relative flex items-center justify-center">
            {/* Soft Lavender Background Disc */}
            <div className="absolute top-[-20px] right-[-20px] w-64 h-64 rounded-full bg-[#ECE5F8]/70 blur-xs pointer-events-none" />

            {/* Circular Plate Image */}
            <div className="relative w-48 h-48 sm:w-56 sm:h-56 rounded-full overflow-hidden shadow-2xl border-4 border-white shrink-0 bg-slate-100 z-10">
              <Image
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 relative z-10">
        {cards.map((card) => (
          ...
          >
            <div>
              {/* Card Top Composition: Round Dish + Plus Badge + Product Item */}
              <div className="relative flex items-center justify-center gap-2 py-4 h-36">
                
                {/* Dish Plate Circle */}
                <div className="relative w-28 h-28 sm:w-32 sm:h-32 rounded-full overflow-hidden shadow-sm border-2 border-white shrink-0 group-hover:scale-105 transition-transform duration-300">
                  <Image
                    src={card.dishImage}
                    alt={card.dishTitle}
                    fill
                ...
                {/* Plus Circle Badge */}
                <div className="w-7 h-7 rounded-full bg-white text-[#3B1E78] font-bold text-xs flex items-center justify-center shrink-0 shadow-sm border border-purple-100/80 z-10 -mx-3">
                  +
                </div>

                {/* Product Item Container */}
                <div className="relative w-18 h-22 sm:w-22 sm:h-26 rounded-2xl overflow-hidden shrink-0 group-hover:scale-105 transition-transform duration-300">
                  <Image
      ...
      <div className="flex items-center justify-center pt-2 relative z-10">
        <button
          onClick={() => onNavigateTab("recommendations")}
          className="px-8 py-3.5 rounded-full bg-white hover:bg-[#F9F7FC] text-[#3B1E78] border border-purple-200/80 font-bold text-xs shadow-2xs hover:shadow-xs flex items-center gap-2 transition-all cursor-pointer active:scale-[0.98]"
        >
