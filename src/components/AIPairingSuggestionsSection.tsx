...
export default function AIPairingSuggestionsSection({ onNavigateTab }: AIPairingSuggestionsSectionProps) {
  const cards = [
    {
      ...
      productSubtitle: "with Black Truffle Oil & Mozzarella",
    ...
    },
    {
      ...
      tag: "Gran Spicco Italy",
      dishTitle: "Mushroom Pasta",
      productSubtitle: "with Shaved Gran Spicco Parmesan",
    ...
    },
    {
      ...
      productSubtitle: "with Norwegian Atlantic Salmon Fillet",
    ...
    },
    {
      ...
      productSubtitle: "with Pure Danish Lurpak Butter",
  ...
  return (
    <div className="w-full bg-[#FAF9FC] rounded-[36px] p-6 sm:p-8 lg:p-10 border border-slate-100 shadow-[0_4px_30px_rgba(0,0,0,0.02)] space-y-9 font-sans text-left relative overflow-hidden">
      ...
      {/* 1. TOP HERO SECTION */}
      <div className="relative flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 z-10 pt-2 min-h-[150px]">
        ...
        <div className="hidden md:flex items-center gap-5 shrink-0 relative z-10">
          ...
          {/* Top-Right Circular Plate Graphic (Kept safely inside flex layout without card overlap) */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 relative z-10">
        {cards.map((card) => (
          <div
            key={card.id}
            className={`${card.bgColor} rounded-[28px] p-5 sm:p-6 border border-slate-200/50 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.03)] hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col justify-between group min-h-[330px] text-left relative overflow-hidden`}
          >
            <div>
              {/* Top Tag Pill */}
              <div className="flex items-center justify-between mb-3">
                <span className="text-[10px] font-bold text-[#3B1E78] bg-white/90 backdrop-blur-xs px-2.5 py-0.5 rounded-full border border-purple-100/80 shadow-2xs">
                  {card.tag}
                </span>
              </div>
